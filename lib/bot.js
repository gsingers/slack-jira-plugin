...
function getBaseUrl(config, prjName) {
  var configUrl;
...
function generateMsg(config, prjName, issue) {
  var configUrl = getBaseUrl(config, prjName);
  return config.emoji + ' <' + configUrl + issue.key + '|*' + issue.key + '*>';
}

function doSearch(splits, self, apis, requests) {
  var prjName = splits.shift();
  var configUrl = getBaseUrl(self.config, prjName);
    ...
    jiraApi.searchJira(searchString, optional, function (error, body) {
      ...
      if (body && body.total > 0) {
        ...
        _.each(body.issues, function (item) {
          ...
          fullMsg += generateMsg(self.config, prjName, item);
              ...
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  if (issue && self.config.post == true) {
                    msg = generateMsg(self.config, prjName, issue);
