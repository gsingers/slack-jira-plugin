  ...
  try {
    jiraApi.searchJira(searchString, optional, function (error, body) {
      ...
      if (body && body.total > 0) {
        ...
        var req = {"channel": chan, "text": fullMsg};
        def.resolve(req)
      } else {
        console.log(error);
        var req = {"channel": null, "text": "There was an error executing your search: " + error};
        def.resolve(req);
      }
    });
  } catch (e) {
    var req = {"channel": null, "text": "There was an error executing your search: " + e};
    def.resolve(req);
  }
            ...
            if (jiraApi && jiraApi != "NONE") {
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  var req = {"channel": message.channel, "text": fullMsg};
                  def.resolve(req);
                });
              })(q.defer(), j);
            } else {
