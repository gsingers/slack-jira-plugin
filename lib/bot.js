...
Bot.prototype.run = function () {
  ...
  bot.use(function (message, cb) {
    if ('message' == message.type && message.text != null && message.subtype != "bot_message") {
      var requests = [];
      if (
          message.text.indexOf("@" + self.config.bot_name) == 0 ||
          message.text.indexOf("@" + self.config.bot_name) == 1 ||
          message.text.indexOf("@" + self.config.bot_id) == 0 ||
          message.text.indexOf("@" + self.config.bot_id) == 1
      ) {
        ...
        while (match = regexp.exec(message.text)) {
          ...
          // (this is async...)
          if (expandInfo === "+" || (self.config.showFullDetailsByDefault && expandInfo !== "-")) {
            if (verbose) {
              console.info("Will show Full details");
            }
            issueDetailsToShow = self.config.fullIssueDetailsToShow;
          } else {
            if (verbose) {
              console.info("Will show Basic details");
            }
            issueDetailsToShow = self.config.basicIssueDetailsToShow;
          }
          var jiraApi = apis[prjName];
          if (jiraApi == null) {
            //explicit urls that don't have jira credentials will be marked by NONE.  If we don't have a
              ...
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  if (issue && self.config.post == true) {
                    ...
                    if (issueDetailsToShow['fields.summary']) {
                      bot.addToMsg(msg, "Summary", issue.fields.summary || issue.key, self.config.maxFieldLength);
                      if (verbose) {
                        console.log(issue);
                      }
                      if (issueDetailsToShow['fields.assignee'] && issue.fields.assignee) {
                        bot.addToMsg(msg, "Assignee", issue.fields.assignee.displayName, self.config.maxFieldLength);

                      }
                      if (issueDetailsToShow['fields.creator'] && issue.fields.creator) {
                        bot.addToMsg(msg, "Creator", issue.fields.creator.displayName, self.config.maxFieldLength);
                      }
                      if (issueDetailsToShow['fields.developer'] && issue.fields.customfield_11041) {
                        bot.addToMsg(msg, "Developer", issue.fields.customfield_11041.displayName, self.config.maxFieldLength);
                      }
                      if (issueDetailsToShow['fields.description'] && issue.fields.description) {
                        bot.addToMsg(msg, "Description", issue.fields.description, self.config.maxFieldLength);
                      }
                    }
                  } else {
  ...
  bot.addToMsg = (function(msg, label, value, maxFieldLength) {
      if (value.length > maxFieldLength) {
        trimmedValue = value.substr(0, maxFieldLength);
        value = value.substr(0, Math.min(trimmedValue.length, trimmedValue.lastIndexOf(" "))) + "...";
      }
      msg.push(self.config.issueFieldSeparator + '*' + label + '*' + self.config.issueLabelSeparator + value );
    })
  );
